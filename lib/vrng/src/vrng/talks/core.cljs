(ns vrng.talks.core
  (:require
   [vrng.talks.progress :as wf :refer [progress-clip-comp]]
   [vrng.talks.timeline :as tl :refer [timeline-comp]]
...
(defn archived-before-planned?
  [starts-at state]
  (and (> (u/to-millis starts-at) (.valueOf (.moment js/window)))
             (= state "archived")))

(defn talk-info-meta-info-comp
  [starts-at state play-count]
  (let [archived-before-planned (archived-before-planned? starts-at state)]
  [:p {:class "meta-info"}
    (if archived-before-planned 
      [:span "Archived"]
      [:span (if (= state "live") "Now" (u/from-now (u/to-millis starts-at)))])
   ...
   [:div#talk-meta.medium-4.medium-offset-1.columns
    [:div {:class "media-object stack-for-small"}
     ...
     [:div {:class "media-object-section pub-meta text-center medium-text-left middle"}
      ...
      [talk-info-meta-info-comp (starts-at) (talk-state) (play-count)]]
     ]]])
...
(defn player-controls-comp []
    [:div.player-container {:class (talk-state)}
     [:div#talk-playbar.row.collapse
      ...
      [timeline-comp (get-in @state [:talk :peaks])]
...
(defn root-comp []
  ...
  (fn []
    [:div.left-side.medium-9.columns.float-left
     [progress-clip-comp (get-in @state [:talk :peaks])]
     ;;[debug-comp]
