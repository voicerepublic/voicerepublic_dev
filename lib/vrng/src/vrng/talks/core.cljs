...
(defn audio-source []
  (or (stream-url) (:mp3 (media-links))))

(defn player-controls-comp []
  (m/source! (audio-source))
  (m/volume! 0.5)
  (fn []
    [:div.player-container {:class (talk-state)}
     [:div#talk-playbar.row.collapse
      ...
      [:span.timecode.jp-current-time (u/format-m-s (:current-time @player))]
