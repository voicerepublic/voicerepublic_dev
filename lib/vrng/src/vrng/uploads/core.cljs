...
(defn sign-file
  ...
  (let [f (file->map (file :file))]
    (GET server-url {:params {:mime-type (f :type)
                              :file-name (f :name)}
                     :handler #(async/put! signed [% file])})))

(defn upload-file
  [[{upload-url :upload-url headers :headers} file]]
  (PUT upload-url {:headers headers
                   :body file
 ...
 [namespace]
  (let [uploader-name (str namespace "-file-input")
        ...
        uploader {:file-input uploader-name
                  :uploaded uploaded
                  :upload-queue (s3-pipe uploaded "http://localhost:3001/sign")}]
