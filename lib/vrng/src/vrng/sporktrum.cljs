...
;; Webkit fix. However:
;; iOS > 11 will not play Audio whatsover if _anything_ is done to a
;; Audio context without explicit user interaction.
(if-not (= (:browser @audio)
           "ios11+")
  (set! (.-AudioContext js/window)
        (or (.-AudioContext js/window)
            (.-webkitAudioContext js/window))))

(if (not (contains? #{"ie" "ios11+"} (:browser @audio)))
...
(defn setup-analyser-for-media-element [id]
  (if-let [element (element-by-id id)]
    (configure (source-from-element element))))
