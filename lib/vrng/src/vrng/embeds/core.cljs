...
(if js/audit (.log js/console "loading vrng.embeds.core"))
...
(defn- responsive-class [width]
  "Returns the responsive class based on a given width. Do not call
  this directly. For performance reasons this is available
  via `(:responsive-class @page-state)`."
  (condp > width
    token  "token"
    tiny   "tiny"
    xs     "xs"
...
(defn more-comp []
  [:div.msg-holder
   ...
   [:a.msg-text
    {:href "/explore" :target "_blank"}
    (case (:responsive-class @page-state)
      "xs"     "More..."
      "small"  "More on VR..."
      "medium" "More on Voice Republic..."
      "There's more on Voice Republic...")] ; else
...
(defn please-login-comp []
  [:div.msg-holder
   [:a.msg-text
    {:href "/users/sign_in" :target "_blank"}
    [:p
     [:span  "Please log in"]]]
   [:a.action-btn.close-msg
    {:title "Cancel"
...
(defn progress-percentage []
  (if (in-talk-state "live" "archived")
