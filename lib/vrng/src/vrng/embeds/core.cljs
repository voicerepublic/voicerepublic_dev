...
(defonce talk
  (atom (js->clj (.-embeddedTalk js/window) :keywordize-keys true)))
...
(defn root-comp []
  [:div.embed-player.clearfix.large
   [:div.image-box [:img {:src (:image_url @talk)}]]
   [:div.content-box
    [:div.top-box.clearfix
     [:div.info-box
      [:a {:href (:self_url @talk) :target "_blank"}
       [:h4.title (:title @talk)]]
      [:div.meta-info
       [:a.user
        {:href (:user_url @talk), :target "_blank"}
        [:span (:user_title @talk)]]
       [:span.date.sm-up " / " (relative-time) " / "]
       [:span.play-count.sm-up (humanreadable-plays) " Plays"]]]
     [:div.branding-box
      [:a {:href (:self_url @talk) :target "_blank"}
       [:div.brand-logo [icon-comp "logo"]]
       [:div.brand-words [icon-comp "logotype"]]]]]
    [:div.player-box
     [:div#jp_jplayer_0.vr-player.jp-jplayer
      ...
      [:audio#jp_audio_0
       {:src "http://localhost:3000/vrmedia/3728.m4a",
        :preload "metadata"}]]
     [:div#jc-3728.jp-audio
      [:div.jp-gui.jp-interface
       [:div.jp-controls
        [:a.jp-pause {:style {:display "none"}} [icon-comp "pause"]]
        [:a.jp-play [icon-comp "play"]]]
       [:div.jp-spacer]
       [:div.jp-volume-bar
       ...
       [:div.embed-msg.hide
        [:a.msg-text
         {:href (:self_url @talk) :title "cancel" :target "_blank"}]
        [:p
       ...
       [:div.teaser-msg.hide
        ...
        [:p
         [:span.translation_missing
          {:title
           "translation missing: en.shared.velvet_player.nice_stuff_on_vr"}
          "Nice Stuff On Vr"]]]
       [:div.jp-progress
        [:div.jp-seek-bar
         {:style {:width "0%"}}
        ...
        [:div.loading-indicator.hide
         [:p
          [:span.translation_missing
           {:title
            "translation missing: en.shared.velvet_player.loading"}
           "Loading"]]]
        [:div.jp-time-holder
       ...
       [:div.action-panel
        [:a.action-btn.slides-btn
         {:href
          "/talks/the-birth-of-comedy-ot-ouf-the-spirit-of-despair",
          :title "View Slides",
          :target "blank"}
         [icon-comp "slides"]]
        [:div.jp-spacer]]
