...
(defn autostart-checkbox-comp []
  [:span.float-right.autostart [:input {:type "checkbox" :on-change toggle-autostart-action :checked (:autostart @page-state)} " autostart"]])

(defn titlebar-comp [title status]
  [:div {:class "titlebar clearfix"}
   [:h4 {:class "title"} title
   ;;TODO Phil: can we do an "if title == 'broadcast' then show this"? I couldn't figure out the syntax.
  [autostart-checkbox-comp]]])
...
(defn device-actionbar-comp []
  [:div {:class "actionbar clearfix"}
   [:div.status-bar {:on-click toggle-mute-action}
     ...
     [:div {:class "holder"}
      [:div.status-bar
       [:span {:class "label scheduled-start"} "Scheduled Start: "
        ...
        ;; --- crudly forced in here
        ;;[:br]
        ;;[:input {:type "checkbox" :on-change toggle-autostart-action :checked (:autostart @page-state)} " autostart"]
        ;; --- end of crudly forced in here
...
(defn mute-comp []
  (if (venue-in-state ["connected"])
    [:span.mute {:on-click toggle-mute-action}
     (if (@page-state :mute)
       [icon-comp "play"]
       [icon-comp "pause"])]))
