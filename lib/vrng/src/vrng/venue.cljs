...
;;    (atom {:context (AudioContext.) :spectrum (js/Uint8Array. 200)})))
...
(defn instructions-for-butt-comp []
  [:div.reveal.medium {:id "instructions", :data-closable ""}
   [:h4 "Instructions"]
   [:p.lead "You need to do some things here..."]
   [:a.button.secondary {:href "./butt"} 
    [:svg 
      {:dangerouslySetInnerHTML { :__html "<use xlink:href='#icon-download' />" } }]"download config file for B.U.T.T."]
   [:p {:class "thing"} [:a.button.hollow.btn-red.btn-hover-yellow {:href "#" :data-close ""} 
     "Cancel"]]])

(defn instructions-for-darkice-comp []
  [:div.reveal.medium {:id "instructions", :data-closable ""}
   [:h4 "Instructions"]
   [:a.button.hollow {:href "./darkice"} "download config file for Darkice"]
   [:p "Run with..."]
   [:code "sudo darkice -c <your-config-file>"]
   [:p "Note: root permissions are require to use posix realtime scheduling."]
   [:p {:class "thing"} [:a.button.hollow.btn-red.btn-hover-yellow {:href "#" :data-close ""} "Cancel"]]])
