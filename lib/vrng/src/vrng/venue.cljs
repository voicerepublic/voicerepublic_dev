...
(defn wizard-page-start-comp []
  [:div#screen1
   ...
   (let [size (reagent/atom 10)
     ...
     [:a {:on-click (fn text-click [e] 
                  (println "margin?")
                  (swap! size + 100))}
      [:h3 {:height "what"}"How will you stream?"]])
...
(defn wizard-page-laptop-comp []
   ...
   [:ul.menu.vertical.medium-horizontal.text-center
    ...
    [:li.box2-select
     ...
     [:a.clearfix
      {:on-click #(select-device-action "darkice")}
      [icon-comp "media"]]
     [:br]
     "DarkIce"]]] )
...
[:div
   ...
   [:ul.menu.vertical.medium-horizontal.text-center
    [:li.box1-select
     ...
     [:a.clearfix
      {:on-click #(start-select-device "bcms")}
      [icon-comp "jeeves"]]
     [:br]
     "B.U.T.T"]
    [:li.box2-select
     ...
     [:a
      {:on-click #(start-select-device "coolmic")}
      [icon-comp "media"]]
     [:br]
     "DarkIce"]]])
