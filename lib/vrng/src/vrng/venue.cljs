...
(defn talks-by-state [talk-state]
  (filter #(= talk-state (:state %)) (vals (:talks (venue)))))
  ...
  [:div {:class "actionbar clearfix"}
   (cond
     ...
     [:div {:class "holder talk-phase2"}
      ...
      [:a {:class (str "button success tiny talk-start-button float-right"
     ...
     [:div {:class "secondary progress talk-progress"
            ...
            :aria-valuemax "100"}
      [:div {:class "progress-meter", :style { :width (active-talk-progress "%")}}
       [:p {:class "progress-meter-text"} [countdown-comp (active-talk-runtime)]]]
      [:a {:href "#"
           ...
           :tabindex "1",
           :title "Are you sure you want to stop the stream early?"} "Stop Broadcast"]])])
...
;; TODO make it have a fixed with so the audio meter doesn't jump
(defn on-air-comp []
...
;; NOTE maybe we should display an approx. delay. later we could use
;; the internal microsphone to calculate the exact delay, if on
...
;; NOTE I think it would suffice to just turn div id: 'instructions'
;; into a modal popup. We could add a cancel button which resets the
;; device selection to none. Clicking cancel will then automatically
;; remove the ;; modal
(defn instructions-for-butt-comp []
  [:div {:id "instructions"}
   [:h4 "Instructions"]
   [:a {:href "./butt"} "download config file for B.U.T.T."]])

(defn instructions-for-darkice-comp []
  [:div {:id "instructions"}
   [:h4 "Instructions"]
   [:a {:href "./darkice"} "download config file for Darkice"]
   [:p "Run with..."]
   [:pre "sudo darkice -c <your-config-file>"]
   [:p "Note: root permissions are require to use posix realtime scheduling."]])
   ...
   [:div {:class "row clearfix"}
    [:div {:id "venue-control-panel", :class "clearfix"}
     ...
     [:div {:class "medium-4 columns talk-panel"}
      ...
      [:div {:class "divider"}]
      [talk-actionbar-comp]]]]
   (if (venue-in-state ["awaiting_stream"]) [instructions-comp])
   [debug-comp]
   ])
