...
(defn update-venue [attributes]
  (swap! state #(assoc-in % [:venue] attributes)))

(defn update-venue-state [state-name]
  (print "Venue state now:" state-name)
  (swap! state #(assoc-in % [:venue :state] state-name)))

(defn update-devices [devices]
  (swap! state #(assoc-in % [:devices] devices)))

(defn venue-message-handler [msg]
  (condp = (:event msg)
    "venue-transition" (do
                         (update-venue (:attributes msg))
                         (update-devices (:devices msg))
