...
(defn time-to-available []
  (* (:availability_countdown @state) 1000))
...
(defn start-talk [talk]
  (PUT (talk-url talk)
       {:format :json
        :params { :talk { :event "start_talk!"}}}))

(defn talk-start-button [talk]
  [:button { :on-click #(start-talk talk) } (t "start")])
   ...
   [:section { :id "claim" :class "dark" }
    [:div { :class "row" }
     [:header
      ...
      (if (some #(= (venue-state) %) ["provisioning" "awaiting_stream"])
        [progress-comp])
      (if (= (venue-state) "offline") [countdown (time-to-available)])
      (if (= (venue-state) "available") [:button { :on-click setup } (t "setup")])
