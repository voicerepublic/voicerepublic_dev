...
(defn venue-provisioning-time []
  (:provisioning_time (venue)))
...
(defn server-progress
  ([] (server-progress ""))
  ([suffix] (str (percentage
                  (- (now) (venue-provisioning-started-at))
                  (venue-provisioning-time)) suffix)))
...
(defn debug-comp []
  [:ul { :style { :color "magenta" } }
   ...
   [:li "runtime: " (active-talk-runtime)]
   [:li "duration: " (active-talk-duration)]
   [:li "server-progress: " (server-progress)]
   [:li "talk-progress: " (active-talk-progress)]
   [:li "provis time: " (venue-provisioning-time)]
   ...
   [:div {:class "row clearfix"}
    [:div {:id "venue-control-panel", :class "clearfix"}
     ...
     [:div {:class "medium-4 columns talk-panel"}
      ...
      [:div {:class "divider"}]
      [talk-actionbar-comp]]]]
   (if (venue-in-state ["awaiting_stream" "disconnected"]) [instructions-comp])
   ;;[debug-comp]
   ])
