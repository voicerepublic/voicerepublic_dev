...
(defn device-status []
  (cond
    ...
    {:class "alert" :reason "Your need to select a streaming source."}
    ...
    (if (= "pair" device-name)
      ...
      (if device-id
        ...
        (do
          ...
          (PUT (venue-url)
               {:format :json
                :params { :venue {:device_name device-name
                                  :event "select_device" }}}))))))
...
(defn retry-select-device-comp []
  (if (@page-state :device-id)
    [:div.spinner.float-right "Look I'm spinning!"]))
  ...
  [:div {:id "page", :on-click close-menu-action}
   ...
   [:div.row.clearfix.control-panels
    [:div {:id "venue-control-panel", :class "clearfix"}
     ...
     [:div.venue-panel
      [:div.device-panel
      [titlebar-comp (t "audio_source") (device-status) retry-select-device-comp]
      ;;[:div {:class "divider"}]
