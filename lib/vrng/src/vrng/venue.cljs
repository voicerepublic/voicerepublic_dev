  ...
  [:div {:class "actionbar clearfix"}
   (cond
     ...
     [:div {:class "holder"}
      ...
      [:p [:a {:class "button success small expanded server-start-button disabled"} "LAUNCH SERVER"]]]
...
(defn instructions-for-butt-comp []
  [:div.reveal.tiny {:id "instructions"}
   ...
   [:p.clearfix [:a.button.small.hollow.btn-white.btn-hover-red.float-left {:href "#" :on-click hide-instructions-action}
     "Cancel"]
...
(defn instructions-for-generic-comp []
  [:div.reveal.medium {:id "instructions"}
   ...
   [:p.text-center [:a.button.small.hollow.btn-red.btn-hover-yellow {:href "#" :on-click hide-instructions-action} "Cancel"]]])
...
(defn instructions-comp []
  (if (and (venue-in-state ["awaiting_stream"]) (:instructions @page-state))
    (condp = (:device-name @page-state)
      "butt"    [:div.instruction-container [instructions-for-butt-comp]]
      "darkice" [:div.instruction-container [instructions-for-darkice-comp]]
      "generic" [:div.instruction-container [instructions-for-generic-comp]]
      nil nil)))
