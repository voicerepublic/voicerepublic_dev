...
(defn close-help-text-action []
  (swap! page-state assoc :help-text-open false))
...
(defn optional-helptext-comp []
  (if (:help-text-open @page-state)
    ...
    [:button.close-button.float-right {:on-click close-help-text-action}
    [:span (goog.string/unescapeEntities "&times;")]]]
    [:p "This is your venue " [:span.strong (venue-name)] 
    " A venue is the digital counterpart to a real stage. For every stage you want to stream, 
    there is one venue page. If you have more than one stage, we’ll help you set up the venues 
    - just contact us under " [generic-help-link-comp "mailto:service@voicerepublic.com" "service@voicerepublic.com"] "."]
    [:p "All talks happening at “venue title” are listed below. 
    You can see and control the streams on this page. 
    Start your server, choose your audio source and then start your broadcast. 
    Any questions? See our help section "  [generic-help-link-comp "http://blog.voicerepublic.com/support" "vr/blog/help"]"."]]][:div]))
