...
(defn show-help-action []
  (track-event "help open")
  (swap! page-state assoc :help false)
  false)


(defn hide-help-action []
  (track-event "help close")
  (swap! page-state assoc :help false)
  false)
...
(defn navbar-comp []
   [:div#navbar.row.expanded
    ...
    [:div#dropdown.small-2.medium-3.columns.text-right
     [:div.avatar.float-right{:style {:background (str "url(" (user-image-url) ")")}}]
...
(defn help-comp []
     [:div#helpbar.row.clearfix {:style 
      {
        :visibility "hidden" 
        :opacity 0
        }
