...
(defn server-status []
  (cond
    ...
    {:class "warning" :reason "The server is currently being started."}
    ...
    {:class "alert" :reason "Less than 5 minutes to scheduled start. Time to launch your server!"}

    (venue-in-state ["available"])
    {:class "warning" :reason "The server is ready to be launched."}

    :else {:class "neutral"}))

(defn server-message-comp [reason]
...
(defn talk-status []
  (cond
    ...
    {:class "alert" :reason "We can't hear anything! Connect your streaming source to start broadcasting."}
...
(defn help-text-trigger-action [event]
  (swap! page-state assoc :instructions true))
...
(defn help-link-comp [page]
  [:a {:target "_blank" :href (blog-url page)} "Need help?"])
...
(defn instructions-for-icast2-comp []
  [:div.medium {:id "instructions"}
   ...
   [:table
    ...
    [:tr [:td "Username"] [:td "source"]]
    [:tr [:td "Password"] [:td (:source_password (venue))]]]
   [help-link-comp "support/streaming-with-icast2-ios/?lang=en"]
   [:p.text-center
    [:a.button.small.hollow.btn-white.btn-hover-yellow
     {:href "#" :on-click hide-instructions-action} "Cancel"]]])
...
(defn optional-helptext-comp []
  [:div {:id "instructions"}
    [:p "This is your venue “venue title”. A venue is the digital counterpart to a real stage. For every stage you want to stream, 
    there is one venue page. If you have more than one stage, we’ll help you set up the venues - just contact us under service@voicerepublic.com. <br><br>
    All talks happening at “venue title” are listed below. You can see and control the streams on this page. Start your server, choose your audio source and then start your broadcast. Any questions? See our help section  [LINK TO HELP AREA LIVE STREAM open in a new tab] ."]])
...
;;Optional Help Text
(defn help-text-trigger-comp []
  [:div 
  [:a.help-text-trigger {:on-click help-text-trigger-action}
  [icon-comp "question"]]])
  ...
  [:div {:id "page", :on-click close-menu-action}
   ...
   [:div.row.clearfix.control-panels
    [:div {:id "venue-control-panel", :class "clearfix"}
     ...
     [:div {:class "medium-4 columns"}
     [:div.talk-panel
      [titlebar-comp (t "broadcast") tinkle (talk-status)]
      [:div {:class "divider"}]
      [talk-actionbar-comp]]
      [server-message-comp]]]]
   ;;[debug-comp]
