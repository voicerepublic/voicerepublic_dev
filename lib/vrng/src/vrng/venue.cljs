...
(defn app-comp [] ; --- mounted to #app
  [:div {:id "page", :on-click close-menu-action}
   ;;[debug-audio-comp]
   ;;[debug-sort-comp]
   (if (@page-state :autostart) [autostart-notice-comp])
   ...
   [:div.row.clearfix.control-panels
    [:div {:id "venue-control-panel", :class "clearfix"}
     ...
     [:div.venue-panel
      [:div.device-panel
      [titlebar-comp (t "audio_source") (device-status) device-control-comp]
      ;;[:div {:class "divider"}]
...
(defn navbar-comp []
   [:div#navbar.row.expanded
    ...
    [:div#titlebar.small-8.medium-6.columns.text-center
     [:div#venue-title "Reeperbahn Festival Bühne 2"]]
    [:div#dropdown.small-2.medium-3.columns.text-right
     [:div.avatar.float-right]
     [:div#help-link [:a.float-right.button {:href "#"} "?"]]]])
   ...
   [:div#talkbar
    [:div#current
     [:p#current-talk-title
      [:span.next-talk "Now: "]
      [:a {:href "#"} "Gorgeous Metal Steering Wheel"]
      [:span.float-right.time-info "00:12"]]]
    [:div#next
     [:p#current-talk-title
      [:span.next-talk "Next: "]
    ...
    [:div#list-toggle
     [:a.float-left.hide-for-small-only
      {:href "#"}
      [:span.show-for-medium.float-left "VIEW ALL"]
      [:svg.icon [:use {:xlink:href "#icon-list"}]]]
    ...
    ;;for each DAY of schedule, one of these blocks:
     ...
     [:div.small-12.columns.talkitem.clearfix
      [:p.clearfix
       [:span.title.float-left.text-left [:a {:href "#"} "title of talk"]]
       [:span.time.float-left.text-left "1.1.1970"]
       [:span.series.float-left.text-left "my series"]
       [:span.duration.float-left.text-right "talk duration"]]]]]
  )
...
(defn stage-comp []
 ...
 [navbar-comp]
  ...
  [talkbar-comp]
   ...
   ;  {:_ "", :style {:visibility "hidden" :opacity 0}}
   ;  [:div#helptext.row
   ;   [:h3 "The Stage Control Panel"]
   ;   [:p
   ;    "This is where you control your stage \"Stage of Nick Niles.\" Each stage of your event has one control panel like this.\n\nAll talks happening at Venue of Nick Niles are listed below. You can see and control the streams on this page. Start your server, choose your streaming source and then start your broadcast. Any questions? Check out the support section."]
   ;   [:h3 "The Server"]
   ;   [:p
   ;    "Blah blah... this is where the rest of the help info should go, item by item."]
   ;   [:button#close-help [:span "×"]]]]
   ; [:div#ui.row.clearfix.expanded
   ;  [:div#left-side.medium-8.columns.no-pad.text-center
   ;   [:div#toggles.clearfix
   ;    [:button.tiny.button.btn-gray.btn-hover-green.hollow.float-left
   ;     [:svg.icon [:use {:xlink:href "#icon-bolt"}]]
   ;     "autostart"]
   ;    [:button.tiny.button.btn-gray.btn-hover-green.hollow.float-right.has-tip.top
   ;     {:aria-haspopup "true",
   ;      :data-yeti-box "k4qmlc-tooltip",
   ;      :tabindex "1",
   ;      :data-n "bpx9ts-n",
   ;      :data-tooltip "",
   ;      :data-resize "k4qmlc-tooltip",
   ;      :data-disable-hover "false",
   ;      :title "",
   ;      :data-toggle "k4qmlc-tooltip",
   ;      :aria-describedby "k4qmlc-tooltip"}
   ;     "sound"
   ;     [:svg.icon [:use {:xlink:href "#icon-sound_on"}]]]]
    ...
    ;      {:href "#"}
    ;      [:span.float-left
    ;       [:svg.icon [:use {:xlink:href "#icon-person"}]]]
    ;      [:span.speaker-name "Don Mattingly"]]]
    ;    [:li.clearfix
    ;     [:a
    ;      {:href "#"}
    ;      [:span.float-left
    ;       [:svg.icon [:use {:xlink:href "#icon-person"}]]]
    ;      [:span.speaker-name "Carl Lewis"]]]
    ;    [:li.clearfix
    ;     [:a
    ;      {:href "#"}
    ;      [:span.float-left
    ;       [:svg.icon [:use {:xlink:href "#icon-person"}]]]
    ;      [:span.speaker-name "Bob Brenley"]]]
    ;    [:li.clearfix
    ;     [:a
    ;      {:href "#"}
    ;      [:span.float-left
    ;       [:svg.icon [:use {:xlink:href "#icon-person"}]]]
    ;      [:span.speaker-name "Darryl Strawberry"]]]
    ;    [:li.clearfix
    ;     [:a
    ;      {:href "#"}
    ;      [:span.float-left
    ;       [:svg.icon [:use {:xlink:href "#icon-person"}]]]
    ;      [:span.speaker-name "Will Clark"]]]
    ;    [:li.clearfix
    ;     [:a
    ;      {:href "#"}
    ;      [:span.float-left
    ;       [:svg.icon [:use {:xlink:href "#icon-person"}]]]
    ;      [:span.speaker-name "Sarah Jessica Parker"]]]
    ;    [:li.clearfix
    ;     [:a
    ;      {:href "#"}
    ;      [:span.float-left
    ;       [:svg.icon [:use {:xlink:href "#icon-person"}]]]
    ;      [:span.speaker-name "Franz Fisch"]]]]]]]]
    ...
    ; }
    ;  [:div#screen1
    ;   [:div.back-close.clearfix [:div.float-right.close-btn "×"]]
    ...
    ;     }
    ;     [:a
    ;      {:href "#"} [icon-comp "megaphone"]]
    ;     [:br]
    ;     "Laptop"]
    ;    [:li.mobile-select.hide
    ;     {:style 
    ;       {
    ;       :padding-top "60px"
    ;       :visibility "hidden"
    ...
    ;    [:div.float-left.back-btn
    ;     [:svg.icon [:use {:xlink:href "#icon-arrow-left"}]]]
    ;    [:div.float-right.close-btn "×"]]
    ;   [:h3 "Which VR Box?"]
    ;   [:ul.menu.vertical.medium-horizontal.text-center
    ;    [:li.box1-select.hide
    ;      {:style 
    ...
    ;        :version "1.1"}
    ;       [:g
    ;        [:defs [:rect#SVGID_1_ {:height "28", :width "28"}]]
    ;        [:clipPath#SVGID_2_
    ;         [:use {:overflow "visible", :xlink:href "#SVGID_1_"}]]
    ;        [:path
    ;         {:d
    ;          "M23,28h5V6h-5V28z M19,3.93c0.861-0.223,1.5-0.998,1.5-1.93c0-1.105-0.895-2-2-2   c-1.104,0-2,0.895-2,2c0,0.932,0.639,1.707,1.5,1.93V6H4V3.93C4.861,3.707,5.5,2.932,5.5,2c0-1.105-0.895-2-2-2   c-1.104,0-2,0.895-2,2c0,0.932,0.639,1.707,1.5,1.93V6H0v22h22V6h-3V3.93z M9,12c1.906,0,3.633,0.764,4.894,2H4.105   C5.369,12.764,7.094,12,9,12 M3.26,15h11.48c0.422,0.605,0.752,1.277,0.967,2H2.295C2.51,16.277,2.838,15.605,3.26,15 M9,26   c-1.906,0-3.631-0.764-4.895-2h9.789C12.633,25.236,10.906,26,9,26 M14.74,23H3.26c-0.422-0.604-0.75-1.277-0.965-2h13.412   C15.492,21.723,15.162,22.396,14.74,23 M15.922,20H2.08C2.033,19.672,2,19.34,2,19s0.033-0.672,0.08-1h13.842   C15.969,18.328,16,18.66,16,19S15.969,19.672,15.922,20 M19,15.5c-0.826,0-1.5-0.672-1.5-1.5s0.674-1.5,1.5-1.5   c0.83,0,1.5,0.672,1.5,1.5S19.83,15.5,19,15.5 M20.5,10c0,0.828-0.67,1.5-1.5,1.5c-0.826,0-1.5-0.672-1.5-1.5   c0-0.828,0.674-1.5,1.5-1.5C19.83,8.5,20.5,9.172,20.5,10",
...
(add-watch state :now-connected
           (fn [key at0m old new]
             (if (and (not= (get-in old [:venue :state]) "connected")
