...
(defn icon-comp [icon]
  [:svg {:dangerouslySetInnerHTML {:__html (str "<use xlink:href='#icon-" icon "' />")}}])
...
;; TODO nick: make it have a fixed with so the other elements don't jump
(defn on-air-comp []
...
(defn talk-listing-talk-comp [talk]
  ...
  [:tr.lineup-list-item
   ...
   [:td.talk-title [:a {:target "_blank" :href (:url talk)} (:title talk)]]
   [:td.blank]])
