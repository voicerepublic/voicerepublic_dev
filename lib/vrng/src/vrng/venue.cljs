...
(defonce page-state (atom {:instructions false
                           :stats {:listener_count 0}}))

(defonce state ; i.e. model-state
...
(defn venue-user-image-url []
  (:image_url (:user (venue))))
...
(defn device-status []
  (cond
    ...
    {:class "success" :reason "The audio source is happly streaming."}
    :else {:class "neutral"}))
...
(defn talk-status []
  (cond
    ...
    {:class "warning" :reason "Time is running out. Less than 10min ti scheduled start."}
...
(defn audio-comp []
  (prn "render audio element")
  (js/setTimeout #(spork/setup-analyser-for-media-element "audio") 500)
  (fn []
    [:audio {:id "audio" :src (icecast-url) :auto-play true :crossorigin "anonymous"}
     [:source {:src (icecast-url) :data-x 1
               :type "audio/ogg" :crossorigin "anonymous"}]]))
     ...
     [:div.holder
      ...
      [:p.button-holder
       ...
       [:a {:href "#"
            ...
            :class "text-link float-right alert talk-stop-button hollow"
            :tabindex "1",
...
(defn instructions-for-butt-comp []
  [:div.reveal.tiny {:id "instructions"}
...
(defn instructions-for-generic-comp []
  [:div.reveal.medium {:id "instructions"}
   ...
   [:p.text-center
    [:a.button.small.hollow.btn-red.btn-hover-yellow
     {:href "#" :on-click hide-instructions-action} "Cancel"]]])
     ...
     [:table
      [:tbody
       [:tr
        ...
        [:th.talk-title "Title"]
        [:th]]
       (doall (map #(map talk-listing-talk-comp (apply sort-by (conj sort-options (talks-by-state %)))) talk-states))]]])) ; ¯\_(ツ)_/¯
...
(defn topbar-comp [] ; --- mounted to #topbar
  ...
  [:div
   [:div#main-menu.top-bar {:class (:class (talk-status))}
    [:div.top-bar-left
     [:ul#branding.menu
      {:role "menubar", :data-dropdown-menu "cblw6x-dropdown-menu"}
    ...
    [:div.top-bar-right.show-for-medium
     [:ul.dropdown.menu
      {:role "menubar",
       :data-dropdown-menu "po8e15-dropdown-menu",
       :data-disable-hover "true",
       :data-click-open "true"}
      [:li.has-submenu.avatar.hide-for-small-only.is-dropdown-submenu-parent.opens-right
       {:aria-label "                            ",
        :aria-expanded "false",
        :aria-haspopup "true",
        :role "menuitem"}
       [:a
        {:tabindex "0", :href "#"}
        [:img.avatar-image {:width "36" :src (venue-user-image-url) :height "36" :alt ""}]]
       [:ul.submenu.menu.vertical.is-dropdown-submenu.first-sub
        {:role "menu", :aria-hidden "true", :data-submenu ""}
        [:li.is-submenu-item.is-dropdown-submenu-item
         ...
         [:a
          ...
          [:span.translation_missing
           {:title
            "translation missing: en.shared.venues_topbar.my_profile"}
           "My Profile"]]]
        [:li.is-submenu-item.is-dropdown-submenu-item
         {:role "menuitem"}
         [:a
          {:href "/talks/new?talk%5Bdryrun%5D=true"}
          [:span.translation_missing
           {:title
            "translation missing: en.shared.venues_topbar.test_talk"}
           "Test Talk"]]]
        [:li.is-submenu-item.is-dropdown-submenu-item
         {:role "menuitem"}
         [:a
          ...
          [:span.translation_missing
           {:title
            "translation missing: en.shared.venues_topbar.schedule_talk"}
           "Schedule Talk"]]]
        [:li.is-submenu-item.is-dropdown-submenu-item
         {:role "menuitem"}
         [:a
          ...
          [:span.translation_missing
           {:title
            "translation missing: en.shared.venues_topbar.upload_talk"}
           "Upload Talk"]]]
        [:li.is-submenu-item.is-dropdown-submenu-item
         {:role "menuitem"}
         [:a
          ...
          [:span.translation_missing
           {:title
            "translation missing: en.shared.venues_topbar.logout"}
           "Logout"]]]]]]]]
   [:div#mobile-nav.show-for-small-only.hide
    [:div.menu.vertical
     [:ul
      [:li
       [:a
        {:href "/"}
        [:span.translation_missing
         {:title "translation missing: en.shared.venues_topbar.home"}
         "Home"]]]
      [:li
       [:a
        {:href "/users/branch14"}
      ...
      [:li
       [:a
        ...
        [:span.translation_missing
         {:title
          "translation missing: en.shared.venues_topbar.create_a_talk"}
         "Create A Talk"]]]
      [:li
       [:a
        {:href "/users/sign_out",
         ...
         :rel "nofollow"}
        [:span.translation_missing
         {:title "translation missing: en.shared.venues_topbar.logout"}
         "Logout"]]]]]]])


(defn modals-comp [] ; --- mounted to #modals
  (if (venue-in-state ["disconnect_required"])
    [instructions-to-disconnect-comp])
  (if (and (venue-in-state ["awaiting_stream"]) (:instructions @page-state))
    ...
    [:div {:class "medium-3 columns dashboard"}
     ...
     (if (< 0 (listener-count))
       [:div {:class "medium-float-right listeners"}
        [:span {:class "listener-count"} (listener-count)]
        [icon-comp "play"]])]]
