  ...
  [:div
   ...
   [:p.clearfix
    [:a.button.small.hollow.btn-white.btn-hover-red.float-left
     {:href "#" :on-click hide-instructions-action} "Cancel"]
    ;; target blank, s.o.
    [:a.button.small.secondary.float-right {:on-click hide-instructions-action
...
(defn device-option-comp [page icon label delay]
  (let [
        offset-y (reagent/atom 50)
        drift (anim/interpolate-to offset-y )
        opacity (reagent/atom 0)
        fade (anim/interpolate-to opacity {:duration 1000})]
    
    (fn []
      [:li.box-select {:on-click #(wizard-page-action page)
                       :style {:opacity @fade 
                               :margin-top @drift}}
       [anim/timeout (fn []
                       (reset! opacity 1)
  ...
  ;         (reset! offset-y 0)
    ...
    [:div#screen1
     ...
     [:ul.menu.vertical.medium-horizontal.text-center
      ...
      ; [:li.box-select {:on-click #(wizard-page-action "box")
      ;                  :style {:opacity @fade 
      ;                          :margin-top @drift
      ;                           }}
      ;  [:div.itemwrapper
      ...
      ;   [:br]
      ;   "StreamBoxx"]]
      ; [:li.laptop-select {:on-click #(wizard-page-action "laptop")
      ;                     :style {:opacity @fade 
      ;                             :margin-top @drift
      ;                           }}
      ;  [:div.itemwrapper
      ...
      ;   [:br]
      ;   "Laptop"]]
      ; [:li.mobile-select {:on-click #(wizard-page-action "mobile")
      ;                     :style {:opacity @fade 
      ;                             :margin-top @drift
      ;                           }}
      ;  [:div.itemwrapper
...
(defn wizard-page-box-comp []
  [:div
   ...
   [:ul.menu.vertical.medium-horizontal.text-center
     ...
     [device-option-comp "box" "box" "New Box" 400]
    ...
    ; [:li.new-box
    ;  [:div.itemwrapper
    ;   {:on-click #(select-device-action "pair")}
    ;   [icon-comp "box"]
    ;   [:br]
    ;   "New Box..."]]
    ]])
...
(defn wizard-page-laptop-comp []
  [:div
   ...
   [:ul.menu.vertical.medium-horizontal.text-center
    [:li {:on-click #(wizard-page-action "butt")}
     [:div.itemwrapper
      [icon-comp "laptop"]
      [:br]
      "B.U.T.T."]]
    [:li {:on-click #(select-device-action "darkice")}
     [:div.itemwrapper
      [icon-comp "laptop"]
      [:br]
      "DarkIce"]]
    [:li.new-box {:on-click #(select-device-action "generic")}
     [:div.itemwrapper
      [icon-comp "laptop"]
      [:br]
      "Another IceCast Client..."]]]])
