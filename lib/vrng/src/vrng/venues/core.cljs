...
(add-server-message "Remove me, I'm just a dummy.")
...
(defn server-message-comp [message]
  ...
  [:div.row.message-row
   [:div.message-losenge.left.float-left
    ...
    [:span.message-time (.format (js/moment (:time message) "x") "H:m")]]])
             ...
             (let [old-state (get-in old [:venue :state])
               ...
               (if (not= old-state new-state)
                 (case [old-state new-state]
                   ...
                   ["offline" "available"]
                   (add-server-message "Launch it!")

                   ["available" "provisioning"]
                   (add-server-message "It's launching...")
