...
(defn end-talk-action [talk]
  (swap! page-state :hide-stop-button true)
      ...
      [:p#current-talk-title
       ...
       (if (onair?)
         [:span.listener-info {:title "listeners"}
          [icon-comp "person"] " " (listener-count)])
...
(defn post-talk-link-comp []
  [:div.clearfix [:a.button.tiny.stage-button {:href (active-talk-url)} [icon-comp "eye"] "View Your Talk"]])
