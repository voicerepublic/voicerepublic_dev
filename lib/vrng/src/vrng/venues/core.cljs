...
;;    [:div#spinner.float-right [icon "loading"]]
;;    [:button#reconnect.float-right.icon-button.large {:on-click reconnect-action} [icon "reload"]]))
      ...
      [:p#current-talk-title
       ...
       (if (onair?)
         [:span.listener-info {:title "listeners"}
          [icon "headphones"] " " (listener-count)])
