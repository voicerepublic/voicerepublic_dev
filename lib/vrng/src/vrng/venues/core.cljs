...
(defn mute-comp []
  (if (venue-in-state ["connected"])
    [:button.tiny.button.btn-gray.btn-hover-green.hollow.mute-toggle
...
(defn device-comp []
  [:button#device-button.tiny.button.btn-gray.btn-hover-green.hollow
   {:on-click toggle-source-wizard-action}
   [icon-comp "soundwave"]
   ;;(first '(device-display-name))
   (subs (device-display-name) 0 16) 
   (if (> (.-length (device-display-name)) 16) "..." "")
   ;;(device-display-name)
   ...
   [:div#current
    (if (active-talk)
      [:p#current-talk-title
       ...
       (if (active-talk-in-state ["live"])
         [:span.listener-info {:title "listeners"}
          [icon-comp "headphones"] " " (listener-count)])
    ...
    (fn []
      [:circle.base
       {:r "110"
        ...
        :stroke-dasharray (source-ring-dasharray 110) ; takes the radius
        :fill "none"
        :cy "50%"
        :cx "50%"
...
(defn autostart-comp []
  ...
  ;;(if (active-talk)
    [:button.tiny.button.btn-gray.btn-hover-green.hollow.text-left
...
(defn toggles-comp []
  [:div#toggles.clearfix
   [:div.small-4.columns.text-left.no-pad
    [autostart-comp]]
   ; [:div.small-4.columns.text-center.no-pad
    ; [device-comp]]
   [:div.small-4.columns.text-right.no-pad
    [mute-comp]]])
   ...
   (let [old-state (get-in old [:venue :state])
     ...
     (if (not= old-state new-state)
       (case [old-state new-state]
         ...
         (add-server-message
          "Your streaming server is launching. This is step 1 of 3. After the server has launched step 2 will be to connect the streaming source. In step 3 you will start the stream.")

         ["awaiting_stream" "connected"]
         (add-server-message
          "Your streaming source is now connected. You are ready to start the stream.")

         ["connected" "disconnect_required"]
         (add-server-message
          (if (venue-device-id)
            "Waiting for disconnect, please stand by..."
            "You need to take action. Please disconnect your streaming client by stopping the broadcast on your device!"))

         ["disconnect_required" "disconnected"]
         (add-server-message
