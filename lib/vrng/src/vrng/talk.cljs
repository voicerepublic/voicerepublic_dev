...
(defn edit-link-comp []
  [:a.button.hollow.muted.tiny.btn-hover-yellow {:href (:edit_url (:talk @state))} "Edit Talk"])

(defn venue-link-comp []
  [:a.button.hollow.muted.tiny.btn-hover-yellow {:href (:url (:venue (:talk @state)))} "Stream This Talk"])
  ...
  [:div.row.collapse.talk-title
   [:div
    ...
    (if js/isHost [:li [edit-link-comp]
      (if (= (talk-state) "prelive") [:li [venue-link-comp]])])]
...
(defn modal-comp [action-name title content-comp]
  [:div.instruction-container
   [:div#instructions.reveal {:role "dialog"}
    [:div {:class "row collapse"}
  ...
  [:div
   [:div {:class "small-6 small-offset-3 columns"}
    [:ul {:class "menu vertical"}
     [:li
      [:a {:class "button small hollow btn-white btn-hover-yellow"}
       [:svg {:dangerouslySetInnerHTML {:__html (str "<use xlink:href='#icon-facebook' />")}}]
       [:span "Facebook"]]]
     [:li
      [:a {:class "button small hollow btn-white btn-hover-yellow"}
       [:svg {:dangerouslySetInnerHTML {:__html (str "<use xlink:href='#icon-twitter' />")}}]
       [:span "Twitter"]]]
     [:li
      [:a {:class "button small hollow btn-white btn-hover-yellow"}
       [:svg {:dangerouslySetInnerHTML {:__html (str "<use xlink:href='#icon-email' />")}}]
       [:span "Mail"]]]]]])
