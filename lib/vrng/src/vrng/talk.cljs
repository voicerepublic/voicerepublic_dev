...
(defn comment-comp [comment]
  ...
  [:div {:class "row collapse comment-block"}
   [:div {:class "comment-meta small-3 columns"}
    ...
    [:p {:class "post-time"} (:created_at comment)]]
...
(defn start-stream []
  (let [url (stream-url)
        player (player)]
    (prn "start playing" (stream-url))
    (.jPlayer player "setMedia" #js {:mp3 url})
    (.jPlayer player "play")))

(defn stop-stream []
  (prn "stop playing" (stream-url))
  (.jPlayer player "stop"))
...
(defn jplayer-config []
  #js {;; --- configuration
       ...
       :ready player-ready
       :error #(prn "error")
       :warning #(prn "WARNING" (-> % .-jPlayer .-warning .-message))
       :suspend #(prn "suspend")
       :play #(prn "play")
       :pause #(prn "pause")
       :loadeddata #(prn "loadeddata")
       :waiting #(prn "waiting")
       :playing #(prn "playing")
       :stalled #(prn "stalled")})
...
(defn add-new-comment [message]
  (let [comment (:message message)]
    ...
    (prn "new comment" comment)
    (prn "comments now:" (comments))))
