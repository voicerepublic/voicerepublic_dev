  ...
  [:div
   [:p
    [:a {:class "button hollow btn-hover-yellow"
     ...
     [:span "Open in Itunes"]]
    [:a {:class "button hollow btn-hover-yellow"
         :href (:https-url @page-state) :target "_blank"}
     [:span "Go to RSS feed"]]
    [:input.form-control {:field :text :id :https-link :disabled true :value (:https-url @page-state)}]]])
