(ns vrng.talk
  (:require
   ...
   ;;[clojure.core.strint :as strint :include-macros true]
   ))


(enable-console-print!)

(defonce page-state (atom {:pinId (.-pinId js/window)}))
...
(defn time-to-start []
  (- (u/to-millis (starts-at)) (* 1000 (:now @state))))
...
(defn user-name []
  (:name (:user (:venue (talk)))))

(defn user-url []
  (:url (:user (:venue (talk)))))

(defn user-image-url []
  (:image_url (:user (:venue (talk)))))

(defn user-image-alt []
  (:image_alt (:user (:venue (talk)))))
...
(defn comments []
  (sort-by :created_at > (:messages (talk))))
...
(defn pin-action []
  (POST (str "/talks/" (:id (talk)) "/reminders")
        {:handler #(swap! page-state assoc :pinId (get % "id"))}))

(defn unpin-action []
  (DELETE (str "/reminders/" (:pinId @page-state))
          {:handler #(swap! page-state assoc :pinId nil)}))
...
(defn edit-link-comp []
  [:a.button.hollow.muted.tiny.btn-hover-yellow {:href (:edit_url (talk))} "Edit Talk"])

(defn venue-link-comp []
  [:a.button.hollow.muted.tiny.btn-hover-yellow {:href (:url (:venue (talk)))} "Stream This Talk"])
  ...
  [:div.row.collapse.talk-title
   [:div
    ...
    [:li
     ...
     ;; FIXME li in li
     (if js/isHost [:li [edit-link-comp]
...
(defn talk-info-comp []
  [:div {:class "row collapse"}
   ...
   [:div#talk-meta.medium-5.columns
  ...
  [:div {:class "player-container"}
   [:div {:class "row collapse", :id "talk-playbar"}
    [:button {:class "button button-playbar button-play-pause state-play btn-hover-light-blue"}
     [:span {:class "jp-play"} [icon-comp "play"]]
     [:span {:class "jp-pause hide", :style { :display "none"}} [icon-comp "pause"]]]
