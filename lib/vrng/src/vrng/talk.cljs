...
(defn title-block-comp []
  [:div.row.collapse.talk-title
   [:div
    [:p.state-container.clearfix
    [:span.label.state-msg.float-left {:class (talk-state)} (talk-state-label)]
    (if js/isHost [:span.clearfix [edit-link-comp] 
      (if (= (talk-state) "prelive") [venue-link-comp])])]
   [:h1 (title)]]])
...
(defn comment-comp [comment]
  ...
  [:div {:class "row collapse comment-block"}
   [:div {:class "comment-meta small-3 columns"}
    [:p
     [:img.comment-avatar {:src (:user_image_url comment) :alt (:user_name comment)}]]
...
(defn modal-comp [action-name title content-comp]
  [:div
   [:div.reveal {:id "share-modal" :role "dialog"}
    [:div {:class "row collapse"}
...
(defn start-stream []
  (let [url (stream-url)
        player (player)]
    (print "start playing" (stream-url))
    (.jPlayer player "setMedia" #js {:mp3 url})
    (.jPlayer player "play")))
