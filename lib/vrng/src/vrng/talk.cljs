...
(defn open-modal-action [action-name]
  (fn []
    (track-event (str "modal open " action-name))
    (swap! page-state assoc action-name true)))

(defn close-modal-action [action-name]
  (fn []
    (track-event (str "modal close " action-name))
...
(defn play-action []
  (track-event "player play talk:" (:id (talk)))
  (print "play-action")
  (start-playing))

(defn pause-action []
  (track-event "player pause talk:" (:id (talk)))
  (print "pause-action")
  (pause))
...
(defn adjust-volume-action [event]
  (track-event "player volume talk:" (:id (talk)))
...
(defn player-controls-comp []
  ...
  [:div.player-container
   [:div#talk-playbar.row.collapse
    [:button.button.button-playbar.button-play-pause.state-play.btn-hover-light-blue
     (if (paused?)
       [:span.jp-play {:on-click play-action} [icon-comp "play"]]
       [:span.jp-pause {:on-click pause-action} [icon-comp "podcast"]])]
    ...
    [:button.button.button-playbar.button-volume.right.hide-for-small-only.btn-hover-red [icon-comp "megaphone"]]
