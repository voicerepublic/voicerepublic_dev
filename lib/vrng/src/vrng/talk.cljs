...
(defn prev-page-action []
  (let [value (max 1 (- (:page @slides) 1))]
    (swap! slides assoc :page value)))

(defn next-page-action []
  (let [value (min (:pages @slides)  (+ (:page @slides) 1))]
    (swap! slides assoc :page value)))

(defn slides-pager-without-callbacks []
  [:div.pager
   [:button.prev {:on-click prev-page-action} "<"]
   [:span (:page @slides)]
   [:span "/"]
   [:span (:pages @slides)]
   [:button.next {:on-click next-page-action} ">"]])

(defn pdf-render-page [page]
  (let [canvas (.getElementById js/document "slides")
        canvas-width (.-width (.getBoundingClientRect canvas))
        viewport (.getViewport page 2.2) ;; WTF why 2.2
...
(defn slides-render-current-page [this]
...
(defn pdf-viewer-without-callbacks []
  [:div.slides
   (if (< (:loading @slides) 1)
     [:div.progress>div.meter
      {:style {:width (str (* 100 (:loading @slides)) "%")}}])
   [:canvas#slides {:style {:width "100%"}}]
   (if (:page @slides) [slides-pager])])
...
(def pdf-viewer
  (with-meta pdf-viewer-without-callbacks
    {:component-did-mount
     (fn [this]
       ;; init pdf viewer
