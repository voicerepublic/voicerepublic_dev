...
(defn stream-offset [] ; in seconds
  (- (:current-time @player) (/ (time-since-start) 1000)))
...
(defn buffered []
  (let [ranges (.-buffered (audio-element))]
    (map (fn [i]
           (let [start (.start ranges i)
                 end (.end ranges i)
                 relative-start (- start (stream-offset))
                 left (u/percentage relative-start (player-duration))
                 width (u/percentage (- end start) (player-duration))]
             {:start start
              :end end
              :left left
              :width width}))
...
(defn play-progress-percentage []
  (str (u/percentage (current-runtime) (player-duration)) "%"))
...
(defn buffered-range-comp [range]
  ^{:key (:start range)}
  [:li {:style {:left (str (:left range) "%")
                :width (str (:width range) "%")
                ;; TODO move css styling to scss files
...
(defn buffered-comp []
  [:ul {:style {:position "relative"}}
   (doall (map buffered-range-comp (:buffered @player)))])
...
(defn player-controls-comp []
  ...
  [:div.player-container {:class (talk-state)}
   [:div#talk-playbar.row.collapse
    ...
    [:span.timecode.jp-current-time (format-m-s (current-runtime))]
    ...
    (if js/signedIn
      ...
      [:button.button.button-playbar.button-pin.hide-for-small-only
       {:on-click (open-modal-action :signin)} [icon-comp "pin"] "Pin"])]]))
...
;;    [:td (str (paused?))]]
   ...
   [:tr
    [:td "current-runtime"]
    [:td (format-m-s (current-runtime))]]
   [:tr
    [:td "time-since-start"]
    [:td (format-m-s (/ (time-since-start) 1000))]]
   [:tr
    [:td "player-duration"]
    [:td (format-m-s (player-duration))]]
   [:tr
    [:td "stream-offset"]
    [:td (format-m-s (stream-offset))]]
   [:tr
    [:td "stream-runtime"]
    [:td (format-m-s (stream-runtime))]]
   [:tr
    [:td {:col-span 2} [buffered-comp]]]
   ])
...
(defn update-player []
  ...
  (if-not (paused?)
    (do
      ...
      ;; update progress
      (swap! player assoc :current-time (stream-runtime))
      (swap! player assoc :buffered (buffered)))))
