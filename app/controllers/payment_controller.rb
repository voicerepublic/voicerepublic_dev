class PaymentController < ApplicationController
  ...
  def user_joined
    ...
    #TODO ersetzen durch validations
    ...
    if participant.update_attributes(:entered_timestamp => timestamp)
      @returncode = "SUCCESS"
  ...
  def user_left
    ...
    if participant.update_attributes(:left_timestamp => timestamp)
      @returncode = "SUCCESS"
  ...
  def payment_started
    ...
    check_checksum(request.url, video_room.video_server.salt, "payment_started", params[:checksum])
    
    if participant.update_attributes(:payment_started_timestamp => timestamp)
      @returncode = "SUCCESS"
    else
      @returncode = "FAILED"
      @reason = "Unsaved"
  ...
  def payment_stopped
    ...
    check_checksum(request.url, video_room.video_server.salt, "payment_stopped", params[:checksum])
    
    if participant.update_attributes(:payment_stopped_timestamp => timestamp)
      @returncode = "SUCCESS"
    else
      @returncode = "FAILED"
      @reason = "Unsaved"
  ...
  def meeting_begin
    ...
    if video_session.update_attributes(:begin_timestamp => timestamp)
      @returncode = "SUCCESS"
    else
      @returncode = "FAILED"
      @reason = "Unsaved"
  ...
  def meeting_end
    ...
    if video_session.update_attributes(:end_timestamp => timestamp)
      @returncode = "SUCCESS"
    else
      @returncode = "FAILED"
      @reason = "Unsaved"
