  ...
  def bookmark_links(user, klu)
    ...
    if user && klu && (klu.user != user) &! klu.user.nil?
      if Bookmark.bookmarked?(user.id, klu.id)
        link_to(user_bookmark_path(:user_id => user, :id => Bookmark.bookmark_for(user.id, klu.id).id), :method => :delete, :title => t('.delete_bookmark_title'), :class => "klu-bookmark-delete") do
          content_tag('i', '', :class => 'icon-bookmark') + content_tag('i', '', :class => 'icon-bookmark-empty')
        end
      else
        link_to(create_bookmark_path(:klu_id => klu), :method => :post, :title => t('.add_bookmark_title'), :class => "klu-bookmark-add") do
          content_tag('i', '', :class => 'icon-bookmark-empty') + content_tag('i', '', :class => 'icon-bookmark') + content_tag(:span, t('.add_bookmark'), :class => 'bookmark-text')
        end
      end
    end
  end
